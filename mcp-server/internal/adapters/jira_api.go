...
func (a *JiraAdapter) ListTicketsByAccount(ctx context.Context, accountSFID string) ([]domain.JiraTicket, error) {
	...
	tickets, err := a.liveListTickets(ctx, accountSFID)
	if err != nil {
		return nil, err
	}

	// Fallback: if the SF-scoped query returned nothing AND a project key is
	// configured, use a project-scoped query so the dashboard shows real data
	// from the configured Jira project even when the custom field is not set.
	if len(tickets) == 0 && a.cfg.JiraProjectKey != "" && accountSFID != "" {
		allTickets, err2 := a.liveProjectTickets(ctx, accountSFID)
		if err2 == nil && len(allTickets) > 0 {
...
// Resolution order:
//  1. If field key resolved → `customfield_10057 = "001..."  ORDER BY updated DESC`
//  2. If field key empty but name configured → `"Salesforce Account ID" = "001..." ORDER BY updated DESC`
//  3. If no field name configured → fall back to project-scoped query.
func (a *JiraAdapter) buildJQL(ctx context.Context, accountSFID string) string {
	fieldKey := a.resolveFieldKey(ctx)

	if fieldKey != "" && accountSFID != "" {
		return fmt.Sprintf(`%s = "%s" ORDER BY updated DESC`, fieldKey, accountSFID)
	}
